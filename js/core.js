...
function createMessageFragment(msg, prevMsg, nextMsg, lastSenderRef) {
    ...
    if (msg.revoked) {
        ...
        revokeDiv.className = 'revoke-tip';
        let senderName = '';
        if (msg.sender === 'user') {
            senderName = '你';
        } else {
        ...
        revokeDiv.innerHTML = `<span style="display:inline-block;background:rgba(0,0,0,0.08);padding:4px 16px;border-radius:12px;font-size:12px;color:var(--text-secondary, #888);letter-spacing:0.3px;text-align:center;white-space:nowrap;">“${senderName}”撤回了一条消息</span>`;
        revokeDiv.style.display = 'flex';
        revokeDiv.style.justifyContent = 'center';
        revokeDiv.style.width = '100%';
        revokeDiv.style.padding = '8px 0';
    ...
    // 撤回按钮（2分钟内，未撤回）
    if (!msg.revoked && msg.sender && (msg.sender === 'user' || msg.sender === settings.partnerName)) {
        const now = Date.now();
        const sentTime = new Date(msg.timestamp).getTime();
        if (now - sentTime <= 120000) {
            actionsHTML += `<button class="meta-action-btn revoke-btn" title="撤回" onclick="revokeMessage('${msg.id}')"><i class="fas fa-undo-alt"></i></button>`;
        }
    }
...
window._triggerPartnerPoke = function() {
    ...
    addMessage({ id: Date.now(), text: pokeText, timestamp: new Date(), type: 'system', revoked: false });
...
function sendBatchMessages() {
    ...
    batchMessages.forEach((msg, index) => {
        setTimeout(() => {
            addMessage({
                id: Date.now() + index, sender: 'user', text: msg.text || '', image: msg.image || null, timestamp: new Date(), status: 'sent', favorited: false, type: 'normal', revoked: false
            });
            playSound('send');
        }, index * 300);
    });
...
window.simulateReply = function() {
    ...
    for (let i = 0; i < replyCount; i++) {
        ...
        setTimeout(() => {
            try {
                ...
                addMessage(msgObj);
                const addedMsg = messages[messages.length - 1];
                if (addedMsg && addedMsg.id) sentMsgIds.push(addedMsg.id);
                ...
                if (shouldSendSticker) {
                    ...
                    setTimeout(() => {
                        ...
                        if (added && added.id) sentMsgIds.push(added.id);
                        playSound('message');
                        if (typeof window._sendPartnerNotification === 'function') {
    ...
    // 自动撤回逻辑（2%~8%概率）
    const totalDelay = replyCount * (settings.replyDelayMin + 8000);
    setTimeout(() => {
        if (sentMsgIds.length === 0) return;
        const targetId = sentMsgIds[Math.floor(Math.random() * sentMsgIds.length)];
        const chance = 0.02 + Math.random() * 0.06;
        if (Math.random() < chance) {
            const revokeDelay = 2000 + Math.random() * 6000;
            setTimeout(() => {
                const msg = messages.find(m => String(m.id) === String(targetId));
                if (msg && !msg.revoked) {
                    const now = Date.now();
                    const sentTime = new Date(msg.timestamp).getTime();
                    if (now - sentTime <= 120000) {
                        ...
                        throttledSaveData();
                        renderMessages(true);
                        showNotification(`${settings.partnerName || '对方'} 撤回了一条消息`, 'info', 3000);
                    }
                }
            }, revokeDelay);
        }
...
// ========== 撤回消息功能 ==========
window.revokeMessage = function(id) {
    const msg = messages.find(m => String(m.id) === String(id));
    if (!msg) {
        alert('消息不存在');
        return;
    ...
    if (msg.revoked) {
        showNotification('消息已被撤回', 'info');
        return;
    }
    const now = Date.now();
    const sentTime = new Date(msg.timestamp).getTime();
    if (now - sentTime > 120000) {
        showNotification('超过2分钟，无法撤回', 'warning');
        return;
    }
    msg.revoked = true;
    throttledSaveData();
