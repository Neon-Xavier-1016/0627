...
const loadData = async () => {
    try {
        ...
        // ===== 后备：从 localStorage 恢复名字（防止刷新后丢失） =====
        if (savedSettings) {
            Object.assign(settings, savedSettings);
        }

        // 优先使用 IndexedDB 中的 settings，localStorage 仅作为后备
        if (!settings.myName || settings.myName === '我') {
            var storedMy = localStorage.getItem('myName');
            if (storedMy) settings.myName = storedMy;
        }
        if (!settings.partnerName || settings.partnerName === '梦角') {
            var storedPartner = localStorage.getItem('partnerName');
            if (storedPartner) settings.partnerName = storedPartner;
        }
...
const saveData = async () => {
    ...
    if (window.moments && typeof window.moments.render === 'function') {
        try {
            const myAv = localStorage.getItem('myAvatar');
            const partnerAv = localStorage.getItem('partnerAvatar');
            if (myAv && window.settings) window.settings.myAvatar = myAv;
            if (partnerAv && window.settings) window.settings.partnerAvatar = partnerAv;
            window.moments.render();
            const topAvatar = document.querySelector('#moments-user-avatar');
            if (topAvatar && myAv) topAvatar.src = myAv;
        } catch (e) { /* 忽略 */ }
    }
};
...
function manageAutoSendTimer() {
    ...
    if (settings.autoSendEnabled) {
        ...
        const updateAvatar = (element, src) => {
            // 判断是“我”还是“对方”的头像
            const isMe = (element === DOMElements.me.avatar);
            const key = isMe ? 'myAvatar' : 'partnerAvatar';
            ...
            if (src) {
                ...
                // 2. 更新 settings（内存）
                if (settings) settings[key] = src;

                // 3. 更新 localStorage（朋友圈读取的源头）
                localStorage.setItem(key, src);

                // 4. 异步保存到 localforage（持久化）
                localforage.setItem(getStorageKey(key), src).catch(() => {});
            } else {
                // 如果传入空值，恢复默认图标并清除存储
                element.innerHTML = `<i class="fas fa-user"></i>`;
                if (settings) delete settings[key];           // 或 settings[key] = null
                localStorage.removeItem(key);
                localforage.removeItem(getStorageKey(key)).catch(() => {});
            }

            // 5. 强制刷新朋友圈（如果已加载）
            if (window.moments && typeof window.moments.render === 'function') {
                setTimeout(() => window.moments.render(), 50);
            }
        };
...
// core.js 底部 DOMContentLoaded 里
document.addEventListener('DOMContentLoaded', function() {
    const chatArea = document.getElementById('chat-container');   // ★ 改这里
    const historyLoader = document.getElementById('history-loader');
...
// ★★★ 全局名字同步函数 ★★★
window.syncNamesToMoments = function() {
    var myName = document.getElementById('my-name')?.textContent?.trim() ||
                 localStorage.getItem('myName') || '我';
    var partnerName = document.getElementById('partner-name')?.textContent?.trim() ||
                      localStorage.getItem('partnerName') || '梦角';
    ...
    // 刷新朋友圈
    if (window.moments && typeof window.moments.forceRender === 'function') {
        var topName = document.querySelector('#moments-user-name');
        if (topName) topName.textContent = myName;
        window.moments.forceRender();
    } else if (window.moments && typeof window.moments.render === 'function') {
        var topName = document.querySelector('#moments-user-name');
        if (topName) topName.textContent = myName;
        window.moments.render();
    }
