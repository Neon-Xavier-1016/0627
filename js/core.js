...
function manageAutoSendTimer() {
    ...
    if (settings.autoSendEnabled) {
        ...
        const updateUI = () => {
            if (!settings.colorTheme) settings.colorTheme = 'gold';
      ...
      function sendMessage(textOverride = null, type = 'normal') {
          ...
          // 图片大小限制（假设 MAX_IMAGE_SIZE 已定义）
          if (imageFile && imageFile.size > (window.MAX_IMAGE_SIZE || 5 * 1024 * 1024)) {
            ...
            const createMessage = (imgSrc = null) => {
                ...
                if (type === 'normal') {
                    ...
                    function _doMarkReadAndStartTyping() {
                       ...
                       if (tiAvatar) {
                           // 获取对方头像图片（假设头像在 #partner-avatar 内）
                           const partnerAvatarDiv = document.getElementById('partner-avatar');
                           const partnerImg = partnerAvatarDiv ? partnerAvatarDiv.querySelector('img') : null;
...
function showModal(modalElement, focusElement = null) {
    if (!modalElement) {
        console.warn('showModal: modalElement 为空');
        return;
    }
    ...
    requestAnimationFrame(() => {
        ...
        if (focusElement && typeof focusElement.focus === 'function') {
            setTimeout(() => {
                try {
                    ...
                    console.warn('focusElement.focus() 失败：', err);
                }
            }, 100);
        }
    });
       ...
       function hideModal(modalElement) {
           // 保护许愿池不被意外关闭
           if (modalElement && modalElement.id === 'wishing-pool-modal') {
               return;
           }
