...
function renderMoodCalendar() {
    ...
    for (let d = 1; d <= daysInMonth; d++) {
        ...
        // ========== 新增：设置整个格子的背景色 ==========
        if (dayData) {
            // 优先使用我的心情颜色，如果没有则使用对方的心情颜色
            let mainMood = null;
            if (dayData.user) {
                mainMood = getAllMoodOptions().find(m => m.key === dayData.user);
            } else if (dayData.partner) {
                mainMood = getAllMoodOptions().find(m => m.key === dayData.partner);
            }

            if (mainMood) {
                dayDiv.style.backgroundColor = mainMood.color + '40';  // 25% 透明度
                dayDiv.style.borderColor = mainMood.color;
