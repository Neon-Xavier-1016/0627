/**
 * features.js — 整合修复版
 * 包含：戳一戳装饰、顶部栏常驻、保活音频、全局搜索、每日公告、沉浸模式、动态名称等
 */

/* ═══════════════════════════════════════════════
   1. 戳一戳装饰符号 — 多选项 + 自定义 + 分角色 + 表情清理
   ═══════════════════════════════════════════════ */
(function() {
...
function _getDailyGreetingData() {
    ...
    var weathers = [
        '晴空万里', '多云转晴', '阴天有云', '细雨蒙蒙', '春风和煦', '微微寒冷', '清风徐徐', '雨后初晴',
        '夜色宁静', '月光皎洁', '晴间多云', '大雨滂沱', '雷雨交加', '小雪纷飞', '微风拂面', '多云天气',
        '雾气朦胧', '星光璀璨', '朝霞满天', '夕阳西下', '海风轻拂', '山间清爽', '秋叶飘落', '花香四溢',
        '绿意盎然', '雨后清新', '雪花飞舞', '阳光明媚'
    ];

    var statusPool = [
        '正在想你 💭', '忙碌中，但心里有你', '好好的，别担心 ✨', '期待见到你', '有点想你了', '在努力变更好',
        '今天挺安静的', '心情不错哦 🌱', '一切都好，你呢？', '看月亮，想到你 🌙', '今天有点想你', '刚刚看到一朵云像你 ☁️',
        '工作再忙也会想你的', '今天你开心吗？', '梦里见 💤', '好好吃饭了吗？', '记得多喝水哦 💧', '今天有没有照顾好自己',
        '想你，但不说 🤫', '全世界你最可爱', '今天天气不错，适合想你', '吃饱喝足，开始想你', '今天也想牵你的手',
        '你有没有想我', '今天比昨天更想你', '看到好吃的想分享给你 🍜', '听到一首歌想到你 🎵', '今天也要加油鸭',
        '晚安，我的全世界 🌙', '早安，又是想你的一天'
    ];
...
function _buildDailyGreeting() {
    try {
        ...
        if (emojiEl && festival) emojiEl.textContent = festival.emoji;

        var moodIconEl = document.getElementById('dg-partner-mood-icon');
        if (moodIconEl && partnerMoodIcon) {
...
window.renderAnnStatusPool = function() {
    ...
    pool.forEach(function(item, idx) {
        ...
        row.style.cssText = 'display:flex;align-items:center;gap:10px;padding:9px 12px;background:linear-gradient(135deg,rgba(var(--accent-color-rgb),0.05),rgba(var(--accent-color-rgb),0.02));border-radius:12px;border:1px solid rgba(var(--accent-color-rgb),0.15);font-size:13px;';
        ...
        row.innerHTML = iconHtml
            ...
            + '</div>'
            + '<button onclick="removeAnnStatusPoolItem(' + idx + ')" style="background:none;border:none;color:var(--text-secondary);cursor:pointer;font-size:14px;padding:3px 5px;border-radius:6px;opacity:0.6;">✕</button>';
...
window.updateDynamicNames = function() {
    try {
        ...
        if (partnerNoRec) partnerNoRec.textContent = pName + ' 这天还没有留下记录';
...
window.hideAppearancePanel = function() {
    document.querySelectorAll('.appearance-sub-panel').forEach(panel => {
...
// 2. 重置外观设置视图（打开模态框时调用，确保显示导航网格）
window.resetAppearanceView = function() {
    const navGrid = document.getElementById('appearance-nav-grid');
    const panelContainer = document.getElementById('appearance-panel-container');
    if (navGrid) navGrid.style.display = 'grid';
    if (panelContainer) panelContainer.style.display = 'none';
...
// 3. 切换外观子面板（核心逻辑）
window.showAppearancePanel = function(panelId) {
    // 隐藏所有子面板
    document.querySelectorAll('.appearance-sub-panel').forEach(panel => {
        panel.style.display = 'none';
    });
    ...
    // ----- 气泡 & CSS 面板（特殊处理，需动态填充内容并绑定事件）-----
    if (panelId === 'bubble-css') {
        targetPanel = document.getElementById('appearance-panel-bubble');
        if (targetPanel) {
            // 填充 HTML 结构
            targetPanel.innerHTML = `
                <div class="settings-section">
                    ...
                    <div class="settings-item-list" style="display:grid;grid-template-columns:1fr 1fr;gap:8px;">
                        <div class="settings-item" data-bubble-style="standard" style="justify-content:center;"><i class="fas fa-comment"></i><span>标准尖角</span></div>
                        <div class="settings-item" data-bubble-style="rounded" style="justify-content:center;"><i class="fas fa-comment-dots"></i><span>圆角</span></div>
                        <div class="settings-item" data-bubble-style="rounded-large" style="justify-content:center;"><i class="fas fa-circle"></i><span>大圆角胶囊</span></div>
                        <div class="settings-item" data-bubble-style="square" style="justify-content:center;"><i class="fas fa-square"></i><span>方形直角</span></div>
                    </div>
                </div>
                    ...
                    <div style="margin-top:14px;">
                        ...
                        <div id="css-live-preview" style="background:var(--chat-bg,var(--primary-bg));border-radius:14px;padding:14px 12px;border:1px solid var(--border-color);">
                            <div style="display:flex;align-items:flex-end;gap:8px;margin-bottom:10px;">
                                <div style="width:32px;height:32px;border-radius:50%;background:var(--accent-color);flex-shrink:0;display:flex;align-items:center;justify-content:center;"><i class="fas fa-user" style="font-size:12px;color:#fff;"></i></div>
                                <div class="message message-received" style="max-width:180px; background: #e4e6eb; padding: 8px 12px; border-radius: 18px; color: #050505;">你是我朝夕相伴触手可及的虚拟</div>
                            </div>
                            <div style="display:flex;align-items:flex-end;gap:8px;justify-content:flex-end;">
                                <div class="message message-sent" style="max-width:180px; background: #0084ff; padding: 8px 12px; border-radius: 18px; color: white;">你是我未曾拥有无法捕捉的亲昵</div>
                                <div style="width:32px;height:32px;border-radius:50%;background:var(--border-color);flex-shrink:0;display:flex;align-items:center;justify-content:center;"><i class="fas fa-user" style="font-size:12px;color:var(--text-secondary);"></i></div>
                            </div>
                        </div>
                    </div>
                </div>
                <div class="settings-section" style="margin-top:18px;">
                    ...
                    <div style="display: flex; justify-content: space-between; align-items:center; gap: 10px; margin-top:8px;">
                        <label style="display:flex;align-items:center;gap:6px;font-size:12px;color:var(--text-secondary);cursor:pointer;">
                            <input type="checkbox" id="global-css-live-toggle" style="width:14px;height:14px;accent-color:var(--accent-color);"> 实时应用
                        </label>
                        <div style="display:flex;gap:8px;">
            ...
            const bubbleTextarea = document.getElementById('custom-bubble-css');
            const styleId = 'dynamic-bubble-css';

            const applyBubbleCSS = (cssText) => {
                ...
                if (typeof settings !== 'undefined') {
                    settings.customBubbleCSS = cssText;
                    if (typeof throttledSaveData === 'function') throttledSaveData();
                }
            };
            ...
            if (applyBtn) {
                applyBtn.onclick = () => {
                    const css = bubbleTextarea ? bubbleTextarea.value : '';
                    applyBubbleCSS(css);
                    if (typeof showNotification === 'function') showNotification('气泡 CSS 已应用', 'success', 1500);
                };
            }
            if (resetBtn) {
                resetBtn.onclick = () => {
                    if (bubbleTextarea) bubbleTextarea.value = '';
                    applyBubbleCSS('');
                    if (typeof showNotification === 'function') showNotification('已清空气泡 CSS', 'info', 1500);
                };
            }
            // 恢复已保存的 CSS
            if (typeof settings !== 'undefined' && settings.customBubbleCSS) {
                if (bubbleTextarea) bubbleTextarea.value = settings.customBubbleCSS;
                applyBubbleCSS(settings.customBubbleCSS);
            }
            // 实时预览：输入时自动应用（防抖）
            if (bubbleTextarea) {
                let timer;
                bubbleTextarea.addEventListener('input', () => {
                    clearTimeout(timer);
                    timer = setTimeout(() => applyBubbleCSS(bubbleTextarea.value), 300);
                });
            }

            // ---------- 绑定全局 CSS 按钮及实时应用 ----------
            const globalApply = document.getElementById('apply-global-css-btn');
            const globalReset = document.getElementById('reset-global-css-btn');
            const globalTextarea = document.getElementById('custom-global-css');
            const globalStyleId = 'dynamic-global-css';
            const globalLiveToggle = document.getElementById('global-css-live-toggle');

            const applyGlobalCSS = (cssText) => {
                let styleEl = document.getElementById(globalStyleId);
                if (!styleEl) {
                    styleEl = document.createElement('style');
                ...
                styleEl.textContent = cssText;
                if (typeof settings !== 'undefined') {
                    settings.customGlobalCSS = cssText;
                    if (typeof throttledSaveData === 'function') throttledSaveData();
                }
                const statusDiv = document.getElementById('global-css-status');
                if (statusDiv) {
                    statusDiv.style.display = 'block';
                    statusDiv.textContent = cssText ? '✓ 已应用全局主题 CSS' : '已清除全局 CSS';
            ...
            if (globalApply) {
                globalApply.onclick = () => {
                    const css = globalTextarea ? globalTextarea.value : '';
                    applyGlobalCSS(css);
                    if (typeof showNotification === 'function') showNotification('全局 CSS 已应用', 'success', 1500);
                };
            }
            if (globalReset) {
                globalReset.onclick = () => {
                    if (globalTextarea) globalTextarea.value = '';
                    applyGlobalCSS('');
                    if (typeof showNotification === 'function') showNotification('已清空全局 CSS', 'info', 1500);
                };
            }
            if (globalLiveToggle) {
                let handler;
                globalLiveToggle.addEventListener('change', () => {
                    if (handler) globalTextarea.removeEventListener('input', handler);
                    if (globalLiveToggle.checked) {
                        handler = () => { if (globalLiveToggle.checked) applyGlobalCSS(globalTextarea.value); };
                        globalTextarea.addEventListener('input', handler);
                    }
                });
            }
            if (typeof settings !== 'undefined' && settings.customGlobalCSS) {
                if (globalTextarea) globalTextarea.value = settings.customGlobalCSS;
                applyGlobalCSS(settings.customGlobalCSS);
            }
            ...
            document.querySelectorAll('[data-bubble-style]').forEach(item => {
                item.onclick = () => {
                    ...
                    switch (style) {
                        case 'standard':
                            css = `.message-sent { border-radius: 16px 4px 16px 16px; } .message-received { border-radius: 4px 16px 16px 16px; }`;
                            break;
                        case 'rounded':
                            css = `.message-sent, .message-received { border-radius: 20px; }`;
                            break;
                        case 'rounded-large':
                            css = `.message-sent, .message-received { border-radius: 40px; }`;
                            break;
                        case 'square':
                            css = `.message-sent, .message-received { border-radius: 0px; }`;
                            break;
                    }
                    if (bubbleTextarea) bubbleTextarea.value = css;
                    applyBubbleCSS(css);
                };
            });
        }
    ...
    // ----- 字体 & 背景面板（同时显示两个子面板）-----
    else if (panelId === 'font-bg') {
        const fontPanel = document.getElementById('appearance-panel-font');
    ...
    // ----- 其他普通面板（theme, avatar 等）-----
    else {
        targetPanel = document.getElementById(`appearance-panel-${panelId}`);
    }

    if (targetPanel) targetPanel.style.display = 'block';
    else if (panelId !== 'font-bg') console.warn(`未找到面板: appearance-panel-${panelId}`);
    ...
    const titleEl = document.getElementById('appearance-panel-title');
    if (titleEl) titleEl.textContent = titleMap[panelId] || panelId;
};

// 4. 绑定外观设置入口（卡片点击）
(function bindAppearanceEntry() {
    const appearanceCard = document.getElementById('appearance-settings');
    if (appearanceCard) {
        appearanceCard.onclick = () => {
            if (typeof resetAppearanceView === 'function') resetAppearanceView();
            const modal = document.getElementById('appearance-modal');
            if (modal && typeof showModal === 'function') showModal(modal);
        };
    } else {
        setTimeout(bindAppearanceEntry, 500);
    }
})();

// 5. 绑定自定义回复按钮（保留原有逻辑）
(function bindCustomReply() {
    function bind() {
        const btn = document.getElementById('custom-replies-function');
        const advModal = document.getElementById('advanced-modal');
        const replyModal = document.getElementById('custom-replies-modal');
        if (btn && advModal && replyModal) {
            btn.onclick = function(e) {
                e.stopPropagation();
                if (typeof hideModal === 'function') hideModal(advModal);
