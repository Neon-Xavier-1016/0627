...
window.closeDailyGreeting = function() {
    try {
        ...
        if (typeof window.triggerPeriodReminder === 'function') {
            window.triggerPeriodReminder();
        }
        // 👇 新增：关闭公告后检测并显示爱心弹窗
        if (typeof window.showImportantDayModalIfNeeded === 'function') {
            window.showImportantDayModalIfNeeded();
        }
    } catch(e) {}
};
